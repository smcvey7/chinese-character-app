...
function SelectedTest({selectedTest, selectedClass, student}){
  const {characters} = useContext(MyContext)
  
  return(
    <table id="selected-test">
        ...
        {selectedTest.items.map((item)=>{
          ...
          return(
            <tr key={uuidv4()} className="studentInfo">
              ...
              <td className={item.choice === character.choices[0] ? "bold studentInfo green-background" : "studentInfo" } value={character.choices[0]} >{character.choices[0]}</td>
              <td className={item.choice === character.choices[1] ? "bold studentInfo red-background" : "studentInfo" } value={character.choices[1]} >{character.choices[1]}</td>
              <td className={item.choice === character.choices[2] ? "bold studentInfo red-background" : "studentInfo" } value={character.choices[2]} >{character.choices[2]}</td>
              <td className={item.choice === character.choices[3] ? "bold studentInfo red-background" : "studentInfo" } value={character.choices[3]} >{character.choices[3]}</td>
              <td className={item.choice === "IDK" ? "bold studentInfo red-background" : "studentInfo" } value="IDK" >IDK</td>
